...
for i in xrange(3): 
    ...
    model.add(Convolution2D(conv1_size, 5, 5,                                    #-- 6 outputs (6 filters), 5x5 convolution kernel
                    border_mode='valid',
    ...
    model.add(Convolution2D(conv2_size, 5, 5, dim_ordering="th"))                                  #-- 16 outputs (16 filters), 5x5 convolution kernel
    model.add(Activation('relu'))                                       #-- ReLU non-linearity
    model.add(MaxPooling2D(pool_size=(2, 2),  dim_ordering="th"))                           #-- A max-pooling on 2x2 windows
    ...
    plt.plot(history.history['val_loss'], label = 'no. of filters = [' + str(conv1_size) + ',' + str(conv2_size) + ']')
